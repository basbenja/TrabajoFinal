...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        \node[node out, right=of N\Nnodlen-1] (y1) {\(\hat{y}_1\)};
        \node[node out, right=of N\Nnodlen-2] (y2) {\(\hat{y}_2\)};
        ...
        % LABELS
        % \node[above=1,align=center,mygreen!60!black] at (N1-1.90) {capa de\\entrada};
        % \node[above=1,align=center,myblue!60!black] at (N3-1) {capas ocultas};
        % \node[above=1,align=center,myred!60!black] at (N\Nnodlen-1.90) {capa\\de salida};
    \end{tikzpicture}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[x=2.7cm,y=1.6cm]
        ...
        \foreach \i in {0,...,\NI} {
            ...
            \else
                \node[node in,outer sep=0.6] (NI-\i) at (0,\y) {\(s^{(1)}_{\i}\)};
            \fi
        }
...
% Introducir Adam y citar el paper
