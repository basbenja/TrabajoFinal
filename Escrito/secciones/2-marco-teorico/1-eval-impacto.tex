...
\begin{comment}
\bigskip
Es importante en este punto identificar claramente los diferentes resultados que hemos
...
\begin{table}[h!]
    ...
    \begin{tabular}{p{7cm}m{7cm}}  % Set fixed column widths
        ...
        \hline
        \textbf{El sesgo de autselección}: el promedio de la diferencia potencial entre lo
        que se observa y lo que se desea medir. & \(\mathbb{E}
        \left[\left(Y_i(1)-Y_i(0)\right) - \left(Y_i(1)-C_i(0)\right)\right] = \mathbb{E}
        \left[C_i(0)-Y_i(0)\right]\) \\
        \hline
    \end{tabular}
    \caption{Distinción entre las diferentes comparaciones presentes en una evaluación de impacto.}
    \label{tab:distinciones}
\end{table}
Teniendo en cuenta las fórmulas expuestas en esta tabla, podemos enunciar de otra forma lo
que dijimos anteriormente: la calidad de una evaluación de impacto dependerá de los
supuestos necesarios para asegurar que no hay sesgo de selección, es decir que
...
\begin{comment}
    ESQUEMA:
    1 - Se propone controlar por un vector de variables observables X. Explicar
    en este punto que para esto sirva y tenga sentido, la decisión  de participación en
    el programa se tiene que deber SOLAMENTE a las características presentes en X
    (supuesto de Independencia Condicional)
    2 - Problema de 1: la cantidad de
    características puede hacerse muy grande.
    3 - Presentar el propensity score o
...
\begin{comment}
A grandes rasgos, la forma en la que se procede al utilizar el PSM es la siguiente:
\begin{enumerate}
    \item Computar el puntaje de propensión de todas las unidades.
    \item Parear las unidades del conjunto de tratamiento con unidades en el conjunto de
    no tratados que tienen los puntajes de propensión más cercanos. Estas unidades
    próximas se convierten en el grupo de control.
    \item Generar la estimación del programa como la diferencia promedio en los resultados
    entre las unidades del tratamiento y sus unidades de comparación correspondientes.
\end{enumerate}
\end{comment}
...
% Hablar de la selección de las variables observables?


% Hablar de cómo se calcula el puntaje de propensión?
...
% Hablar de cómo se hace el emparejamiento?
...
comparación pareadas que puedan influir en el resultado. \footnotetext{Si bien es deseable
que se cumpla el supuesto de SC, para ``solucionarlo'' basta con restringir la muestra a
aquellos individuos tratados para los que sí se pudo encontrar un control adecuado.}
